...
# example:
# if debian, echo -en "write 2+2\nwrite hello world\ny\n" | TEMP=0 JIT=1 METAL_XCODE=1 python3 coder.py
...
if __name__ == "__main__":
    ...
    def fix_bf16(weights):
        return {
            k: (
                v.to(Device.DEFAULT).cast(dtypes.float16)
    ...
    toks = [spp.bos_id()] + encode_prompt(
        "system",
        "You are a useful assistant who writes Python code to answer questions. You keep the code as short as possible and does not read from user input",
    )
